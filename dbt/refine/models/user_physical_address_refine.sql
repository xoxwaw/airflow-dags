WITH upa AS (SELECT user_id                  AS user_record,
             ...
             FROM `ingest.user_physical_address`
             WHERE is_mailing = 0),
     upa1 AS (SELECT user_id              AS user_record,
...
SELECT um.user_record,
       ...
       ""                      AS country_code
FROM (SELECT user_record
      FROM `refine.user_mapping`
      WHERE user_record IS NOT NULL) AS um
         LEFT JOIN
     upa
